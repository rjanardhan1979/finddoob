...
async def index(request: Request):
    startbtnText = 'Show'
    input_names = [0,1]    
    context = {'request': request, 'startbtnText': startbtnText, 'input_names': input_names} 
...
async def txtchange(request: Request): 
    ...
    context = {'request': request, 'getbtnText': getbtnText, 'visibility': visibility}
...
async def index(request: Request, db: SessionLocal = Depends(get_db)):  
    ...
    context = {'request': request, 'df': mydf, 'markets': markets, 'product_line': product_line, 'brands': brands, 'sub_brands': sub_brands, 'components': components}
...
async def index(request: Request, db: SessionLocal = Depends(get_db)):
    ...
    context = {'request': request, 'array': cols,'markets': markets, 'product_line': product_line, 'brands': brands, 'sub_brands': sub_brands, 'components': components}
...
async def skuselect(request: Request, db: SessionLocal = Depends(get_db)):    
    ...
    sales = db.query(models.Sales).filter(models.Sales.sku == val).options(load_only('y_2017', 'y_2018','y_2019', 'y_2020','y_2021', 'y_2022','y_2023', 'y_2024','y_2025', 'y_2026','y_2027', 'y_2028', 'y_2029', 'y_2030'))
    sls = pd.read_sql(sales.statement,engine, index_col='id')
...
async def noteditable(request: Request, id:str, component_id: str, beforeVal: str, field:str, db: SessionLocal = Depends(get_db)):
    ...
    update_dict = {'table_id': id, 'field_name': field, 'old_value': beforeVal, 'new_value': afterVal, 'changed_on': str(datetime.now())}
    if afterVal != beforeVal:
        db.add(models.Audit(**update_dict))
        db.query(models.Sales).filter(models.Sales.id == id).update({field: afterVal})
        db.commit()
        c2 = table_refresh(request, 'record', '', db, '', id)['context_oob']

    c1 =   {'request': request,'component_id': component_id, 'afterVal': afterVal, 'id': id, 'field': field}
    if c2 != None:
        context = {**c1, **c2} 
    else:
        context = c1  
...
def chart_func(table_content):    
    table_content_df = pd.read_sql(table_content.statement,engine, index_col='id')
    yrs = columns()['col_data_only']
    yrs_x = columns()['graph_header_list']    
...
def columns():
    ...
    col_table_show = [ 'id','sku', 'sku_desc', 'cost','asp','margin', 'y_2017', 'y_2018','y_2019', 'y_2020','y_2021', 'y_2022','y_2023', 'y_2024','y_2025', 'y_2026','y_2027', 'y_2028', 'y_2029', 'y_2030']
    col_data_only = ['y_2017', 'y_2018','y_2019', 'y_2020','y_2021', 'y_2022','y_2023', 'y_2024','y_2025', 'y_2026','y_2027', 'y_2028', 'y_2029', 'y_2030']
    graph_header_list = ['Y2017', 'Y2018','Y2019', 'Y2020','Y2021', 'Y2022','Y2023', 'Y2024','Y2025', 'Y2026','Y2027', 'Y2028', 'Y2029', 'Y2030']    
    context = {'col_dict': col_dict, 'col_table_show': col_table_show, 'col_data_only': col_data_only, 'graph_header_list': graph_header_list}
    return context
...
def table_refresh(request, req_type, params, db, component, id):
    ...
    context = {'request': request, 'table_contents': table_content_dict, 'mydf_diff':mydf_diff, 'mydf_diff_colors': mydf_diff_color, 'sparkline': chart_func(table_content), 'cols': cols} 
    response = templates.TemplateResponse("showTable.html", context)
    context_oob = {'table_contents': table_content_dict, 'mydf_diff':mydf_diff, 'mydf_diff_colors': mydf_diff_color, 'sparkline': chart_func(table_content), 'cols': cols}
